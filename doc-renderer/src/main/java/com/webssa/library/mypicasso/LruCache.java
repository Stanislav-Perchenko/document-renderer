...
import static com.webssa.library.mypicasso.Utils.KEY_SEPARATOR;
...
public class LruCache implements Cache {
  ...
  @Override public void set(String key, Bitmap bmp) {
    ...
    try {
      ...
      String msg = String.format("{orig exception message = '%s'; key='%s', bmpW=%d, bmpH=%d, bmpSize=%d}", e.getMessage(), key, bmp.getWidth(), bmp.getHeight(), Utils.getBitmapBytes(bmp));
    ...
    while (true) {
      ...
      synchronized (this) {
        if (size < 0 || (map.isEmpty() && size != 0)) {
          String msg = String.format("{%s.sizeOf() is reporting inconsistent results! size=%d, storedItemsCount=%s}", getClass().getSigners(), size, map.size());
          throw new IllegalStateException(msg);
        }
